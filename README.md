...
- CPG construction now includes a small verified component-edge merge from
  AST/CFG/PDG-style edges into typed CPG edges, but real AST/CFG/DDG/CDG
...
- CPG node predicates: traversal templates can require checked source/sink
  classes, call names, node kinds, and argument-index facts.
- CPG policy provenance: source/sink class facts can be backed by call-name facts
  and explicit source/sink policy rules.
- CPG sanitizer policy: sanitizer call-name facts and sink-class requirements
  connect CPG traversal evidence to the context-sensitive sanitizer lattice.
- CPG ordered sanitizer evidence: sanitizer nodes must precede the sink and share
  a checked flow token with the sink input.
- CPG sanitizer triage: ordered sanitizer evidence can become proof-carrying
  suppression and compose with the CI no-bug-hiding gate.
...
- multi-finding source-backed no-orphan CPG reports: aggregate source-level CPG
  ...
  checked entry.
- source-backed no-orphan CPG CI gate: multi-finding source CPG reports compose
  with `ci_gate_no_bug_hiding` while retaining per-entry hop-origin evidence.
