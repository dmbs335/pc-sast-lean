...
| IFDS/CPG certificates | Verified/conditional | Accepted path/fixpoint/summary certificates imply trusted graph facts; sparse IFDS fact-flow relations lower to exploded graph paths; component CPG edges merge into typed CPG paths; CPG traversal templates check edge-kind and policy-backed node-property patterns | That a real extractor built the graph correctly |
